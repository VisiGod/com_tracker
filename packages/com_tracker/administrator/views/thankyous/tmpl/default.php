...
if ($saveOrder)
{
	$saveOrderingUrl = 'index.php?option=com_tracker&task=thankyous.saveOrderAjax&tmpl=component';
	JHtml::_('sortablelist.sortable', 'thankyouList', 'adminForm', strtolower($listDirn), $saveOrderingUrl);
}
...
<form action="<?php echo JRoute::_('index.php?option=com_tracker&view=thankyous'); ?>" method="post" name="adminForm" id="adminForm">
	...
	<div id="j-main-container" class="span10">
		...
		<table class="table table-striped" id="thankyouList">
			<thead>
				<tr>
				...
				<th class="nowrap">
					<?php echo JHtml::_('grid.sort',  'COM_TRACKER_TORRENT_NAME', 'tt.name', $listDirn, $listOrder); ?>
				</th>

				<th class="nowrap">
					<?php echo JHtml::_('grid.sort',  'JGLOBAL_USERNAME', 'du.username', $listDirn, $listOrder); ?>
				</th>
			...
			?>
				...
				<td>
				<?php
					if ($canEdit) echo "<a href=".JRoute::_('index.php?option=com_tracker&task=thankyou.edit&id='.(int) $item->id).">".$this->escape($item->id)."</a>";
					else echo $item->id;
				?>
				</td>
				...
				<td><?php echo $this->escape($item->created_time); ?></td>

				<?php if (isset($this->items[0]->state)): ?>
					<td class="center">
						<?php echo JHtml::_('jgrid.published', $item->state, $i, 'thankyous.', $canChange, 'cb'); ?>
					</td>
                <?php endif; ?>
