...
if ($saveOrder)
{
	$saveOrderingUrl = 'index.php?option=com_tracker&task=rsses.saveOrderAjax&tmpl=component';
	JHtml::_('sortablelist.sortable', 'rssList', 'adminForm', strtolower($listDirn), $saveOrderingUrl);
}
...
<form action="<?php echo JRoute::_('index.php?option=com_tracker&view=rsses'); ?>" method="post" name="adminForm" id="adminForm">
	<div id="j-main-container">
		...
		<table class="table table-striped" id="rssList">
			<thead>
				<tr>
				...
				<th class="nowrap">
					<?php echo JHtml::_('grid.sort',  'COM_TRACKER_RSS_NAME', 'a.name', $listDirn, $listOrder); ?>
				</th>

				<th>
					<?php echo JHtml::_('grid.sort',  'COM_TRACKER_RSS_CHANNEL_TITLE', 'a.channel_title', $listDirn, $listOrder); ?>
				</th>

				<th>
					<?php echo JHtml::_('grid.sort',  'COM_TRACKER_RSS_AUTHENTICATION', 'a.rss_authentication', $listDirn, $listOrder); ?>
				</th>

				<th>
					<?php echo JHtml::_('grid.sort',  'COM_TRACKER_RSS_TYPE', 'a.rss_type', $listDirn, $listOrder); ?>
				</th>

				<th>
					<?php echo JHtml::_('grid.sort',  'COM_TRACKER_RSS_ITEM_COUNT', 'a.item_count', $listDirn, $listOrder); ?>
				</th>
			...
			?>
				...
				<td>
				<?php
					if ($canEdit) echo "<a href=".JRoute::_('index.php?option=com_tracker&task=rss.edit&id='.(int) $item->id).">".$this->escape($item->id)."</a>";
					else echo $item->id;
				?>
				</td>
				...
				<td><?php echo $this->escape($item->channel_title); ?></td>
				...
				<?php if (isset($this->items[0]->state)): ?>
					<td class="center">
						<?php echo JHtml::_('jgrid.published', $item->state, $i, 'rsses.', $canChange, 'cb'); ?>
					</td>
                <?php endif; ?>
