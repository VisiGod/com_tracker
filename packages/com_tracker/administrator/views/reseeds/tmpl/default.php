...
if ($saveOrder)
{
	$saveOrderingUrl = 'index.php?option=com_tracker&task=reseeds.saveOrderAjax&tmpl=component';
	JHtml::_('sortablelist.sortable', 'reseedList', 'adminForm', strtolower($listDirn), $saveOrderingUrl);
}
...
<form action="<?php echo JRoute::_('index.php?option=com_tracker&view=reseeds'); ?>" method="post" name="adminForm" id="adminForm">
	...
	<?php endif;?>
		...
		<table class="table table-striped" id="reseedList">
			<thead>
				<tr>
			...
			?>
				...
				<td>
				<?php
					if ($canEdit) echo "<a href=".JRoute::_('index.php?option=com_tracker&task=reseed.edit&id='.(int) $item->id).">".$this->escape($item->id)."</a>";
					else echo $item->id;
				?>
				</td>
                ...
                <?php if (isset($this->items[0]->state)): ?>
					<td class="center">
						<?php echo JHtml::_('jgrid.published', $item->state, $i, 'reseeds.', $canChange, 'cb'); ?>
					</td>
                <?php endif; ?>
